...
TREZOR_FIRMWARE_DEPENDENCIES = python3 host-python3 host-python3-setuptools

# Build the Core emulator
define TREZOR_FIRMWARE_BUILD_CMDS
	cd $(@D)/core && \
		$(HOST_DIR)/bin/python3 -m pip install --user pipenv && \
		$(HOST_DIR)/bin/python3 -m pipenv install && \
		$(HOST_DIR)/bin/python3 -m pipenv run make build_unix
endef
